...
import com.example.lock.zk.IZk;
import com.example.lock.zk.Lock;
import com.example.lock.zk.WaitLock;
 ...
 */
public class ZkNodeLock implements Lock, WaitLock, Watcher {
    ...
    public boolean waitLock(final String node, final int sessionTime) {
        try {
            ...
            if (!Objects.isNull(stat)) {
                ...
                //3s
                await.await(sessionTime, TimeUnit.MICROSECONDS);
            }
        } catch (KeeperException e) {
            e.printStackTrace();
            return false;
